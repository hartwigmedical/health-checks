...
public class InsertSizeMetricsExtractor extends AbstractDataExtractor {
    ...
    private static final String MAPPING_INSERT_SIZE_MEDIAN = "MAPPING_INSERT_SIZE_MEDIAN";
    ...
    private List<BaseDataReport> getSampleData(final String runDirectory, final String sampleType)
        ...
        final String patientId = getPatientId(runDirectory, suffix, lines, "INPUT");

        final List<BaseDataReport> baseDataReports = new ArrayList<>();
        final BaseDataReport baseDataReport = getValue(runDirectory, suffix, lines, MEDIAN_INSERT_SIZE, patientId);
    ...
    private String getPatientId(final String runDirectory, final String suffix, final List<String> lines,
                    final String filter) throws LineNotFoundException {
        final Integer index = findLineIndex(runDirectory, suffix, lines, filter);
    ...
    private BaseDataReport getValue(final String runDirectory, final String suffix, final List<String> lines,
                    final String filter, final String patientId) throws LineNotFoundException {
        final Integer index = findLineIndex(runDirectory, suffix, lines, filter);
        final String line = lines.get(index + ONE);
        final String[] values = line.split(SEPERATOR_REGEX);
        final BaseDataReport baseDataReport = new BaseDataReport(patientId, MAPPING_INSERT_SIZE_MEDIAN, values[ZERO]);
        return baseDataReport;
    }

    private Integer findLineIndex(final String runDirectory, final String suffix, final List<String> lines,
                    final String filter) throws LineNotFoundException {
        ...
        if (!lineNumbers.isPresent()) {
            throw new LineNotFoundException(String.format(LINE_NOT_FOUND_ERROR, suffix, runDirectory, filter));
        }
