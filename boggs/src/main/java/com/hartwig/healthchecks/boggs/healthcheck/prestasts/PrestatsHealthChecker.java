...
public class PrestatsHealthChecker implements HealthChecker {
    ...
    public BaseReport runCheck() throws IOException, EmptyFileException {
        ...
        prestatsReport.getSummary().forEach((dataReport) -> {
            if (dataReport.getStatus().equalsIgnoreCase(FAIL_ERROR)) {
                LOGGER.info(String.format(FOUND_FAILS_MSG, dataReport.getCheckName(), dataReport.getFile()));
            }
        });
